...
export const register = ({ email, password }) => dispatch => {
    ...
    axios.post(`${authenticator}/auth/register`, body, config)
        .then(res => dispatch({
...
export const loadUser = () => (dispatch, getState) => {
  ...
  axios.get(`${authenticator}/auth/profile`,tokenConfig(getState))
      .then(res => dispatch({
        type: USER_LOADED,
        payload: res.data
      }))
...
export const login = ({email, password}) => dispatch => {
    ...
    // Request body
    const body = JSON.stringify({email, password})
    axios.post(`${authenticator}/auth/login`, body, config)
        .then(res => dispatch({
            type: LOGIN_SUCCESS,
            payload: res.data
        }))
...
export const tokenConfig = getState => {
    ...
    const token = getState().auth.token; //auth reducer
